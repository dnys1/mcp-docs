...
export async function parseLlmsTxt(url: string): Promise<LlmsTxtEntry[]> {
...
export async function fetchLlmsTxtDocs(
  llmsTxtUrl: string,
  options?: { includeOptional?: boolean },
): Promise<FetchedDocument[]> {
  const entries = await parseLlmsTxt(llmsTxtUrl);

  const filteredEntries = options?.includeOptional
    ? entries
    : entries.filter((e) => !e.isOptional);

  console.log(`Fetching ${filteredEntries.length} documents from llms.txt...`);

  const documents: FetchedDocument[] = [];

  for (const entry of filteredEntries) {
    try {
      const doc = await fetchDocument(entry);
      documents.push(doc);
...
async function fetchDocument(entry: LlmsTxtEntry): Promise<FetchedDocument> {
  let url = entry.url;

  // URLs should already be resolved to absolute in parseLlmsTxt
